import yaml
from scripts.B4.b4_train import train 
...
# select config path
config_path = 'configs/b4_config1.yaml'
...
# model
model = Baseline4(fine_tune_all=False,num_classes=len(config_dict['dataset']['classes'])).to(device)
...
train_dataset = ImageLevelDataset(input_root,annot_file,categories_dict,train_ids,train_transform,one_frame=False)
val_dataset = ImageLevelDataset(input_root,annot_file,categories_dict,val_ids,val_transform,one_frame=False)
test_dataset = ImageLevelDataset(input_root,annot_file,categories_dict,test_ids,val_transform,one_frame=False)

num_workers = config_dict['train']['num_workers']

train_loader = DataLoader(train_dataset,
                          batch_size=config_dict['train']['batch_size']['train'],
                          shuffle=True,
                          num_workers=num_workers,
                          pin_memory=True)
val_loader = DataLoader(val_dataset,
                        batch_size=config_dict['train']['batch_size']['val'],
                        shuffle=False,
                        num_workers=num_workers,
                        pin_memory=True)
